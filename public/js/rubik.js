  ...
  waitForThree(function () {
    ...
    class Storage {
      ...
      loadPreferences() {
        try {
          ...
          // this.game.themes.setTheme(preferences.theme); // Jangan panggil di sini
          ...
          this.game.themes.setTheme("cube"); // Gunakan default theme
    ...
    class Themes {
      ...
      setTheme(theme = false, force = false) {
        ...
        // Pastikan elemen DOM sudah ada sebelum mengakses style
        if (this.game.dom && this.game.dom.prefs) {
          const handles = this.game.dom.prefs.querySelectorAll(".range__handle div");
          handles.forEach((range) => {
            range.style.background = "#" + colors.R.toString(16).padStart(6, "0");
          });
        }

        if (this.game.cube && typeof this.game.cube.updateColors === "function") {
          this.game.cube.updateColors(colors);
        }

        if (this.game.confetti && typeof this.game.confetti.updateColors === "function") {
          this.game.confetti.updateColors(colors);
        }

        if (this.game.dom && this.game.dom.back) {
    ...
    class Game {
      ...
      initActions() {
        ...
        // Di dalam initActions(), pastikan event listener terpasang dengan benar:
        this.dom.game.addEventListener(
          "click",
          (event) => {
            console.log("Game clicked!"); // Pastikan ini muncul di console
    ...
    // Seluruh kode JS Rubik's Cube Anda di sini
    // ...existing code...
    // (Paste seluruh isi <script> Rubik's Cube Anda di sini)
    // ...existing code...
  });
});
